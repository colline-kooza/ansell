...
import {
  LayoutDashboard, Heart, Briefcase, Newspaper, Home, LogOut, Settings,
  ChevronRight, PanelLeft, Menu, Bell, Search, User, FileText, Star,
} from "lucide-react";
import {
...
/* ── Nav Leaf ────────────────────────────────────────────────────────────── */
function NavLeaf({ href, label, icon: Icon, badge }: NavItemDef) {
  ...
  return (
    ...
    >
      ...
      {badge != null && badge > 0 && (
        <span className="ml-auto flex h-4 min-w-4 items-center justify-center rounded-full bg-primary text-[9px] font-bold text-white px-1">
          {badge}
        </span>
      )}
    </Link>
      ...
      items: [
        {
          ...
          icon: Heart,
          badge: favoritePropertyIds.length,
        },
        {
          label: "Saved Jobs",
          href: "/user/dashboard/saved-jobs",
          icon: Star,
          badge: favoriteJobIds.length,
        },
      ],
    },
    {
      ...
      <div className="flex items-center gap-2">
        <div className="relative hidden md:flex items-center">
          ...
          <input
            ...
            className="h-9 w-[200px] lg:w-[240px] rounded-xl bg-gray-50 pl-9 pr-4 text-[13px] text-gray-700 placeholder:text-gray-400 outline-none focus:ring-2 focus:ring-primary/20 transition-all"
          />
        </div>
        ...
        {totalFavorites > 0 && (
          <Link
            href="/user/dashboard/saved-properties"
            className="relative flex h-9 w-9 items-center justify-center rounded-xl text-gray-400 hover:bg-gray-100 hover:text-red-500 transition-colors"
          >
...
export default function UserDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      >
        <UserNavbar onToggleCollapse={() => setCollapsed((c) => !c)} />
