...
import { useAdminInquiries, type PropertyInquiry } from "@/hooks/use-properties";
...
import { Search, MessageSquare, Eye, Mail, Phone, Building2 } from "lucide-react";
...
export default function AdminInquiriesPage() {
  ...
  const { data, isLoading } = useAdminInquiries({ page, page_size: 25 });

  const inquiries = (data?.data ?? []).filter((inq) => {
    if (!search) return true;
    const s = search.toLowerCase();
    return (
      inq.name.toLowerCase().includes(s) ||
      inq.email.toLowerCase().includes(s) ||
      inq.property?.title?.toLowerCase().includes(s)
    );
  });
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold tracking-tight text-gray-900">Property Inquiries</h1>
          <p className="text-xs text-gray-500 mt-0.5">All messages submitted through property listings</p>
        </div>
      </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row justify-between gap-3">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <Input placeholder="Search by name, email, property..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
            </div>
          </div>
        </CardHeader>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  inquiries.map((inq) => (
                    <TableRow key={inq.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setViewing(inq)}>
                      <TableCell>
                        <div>
                          <p className="text-[12px] font-semibold text-gray-900">{inq.name}</p>
                          <p className="text-[10px] text-gray-400">{inq.email}</p>
                        </div>
                      </TableCell>
                      <TableCell className="text-[11px] text-gray-600 max-w-[150px] truncate">
                        {inq.property?.title || "—"}
                      </TableCell>
                      <TableCell className="text-[11px] text-gray-500 max-w-[200px] truncate">
                        {inq.message}
                      </TableCell>
                      ...
                      <TableCell>
                        <Badge className={cn("text-xs border", inq.is_read ? "bg-gray-50 text-gray-500 border-gray-200" : "bg-primary/10 text-primary border-primary/20 font-semibold")}>
                          {inq.is_read ? "Read" : "New"}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="h-8 w-8 p-0" onClick={(e) => { e.stopPropagation(); setViewing(inq); }}>
        ...
        {viewing && (
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                {new Date(viewing.created_at).toLocaleDateString("en-GB", { dateStyle: "long" })}
              </DialogDescription>
            </DialogHeader>
            <Separator />
              ...
              {viewing.property && (
                <div className="p-3 rounded-xl bg-gray-50 border border-gray-100">
                  ...
                  <p className="text-sm font-semibold text-gray-900 flex items-center gap-1.5">
                    <Building2 className="h-4 w-4 text-primary" />{viewing.property.title}
                  </p>
                </div>
              )}
